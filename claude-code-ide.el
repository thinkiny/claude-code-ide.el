...
(defun claude-code-ide--get-process-descendants (pid)
  "Recursively get all descendant PIDs of process PID.
Returns a list of all child and grandchild process IDs.
Uses pgrep on Unix-like systems.  Returns empty list if pgrep unavailable."
  (when (and pid (memq system-type '(darwin gnu/linux)))
    (condition-case err
        (let* ((output (shell-command-to-string (format "pgrep -P %d" pid)))
               (children (when (and output (not (string-empty-p output)))
                           (mapcar #'string-to-number
                                   (split-string (string-trim output) "\n" t)))))
          ;; Recursively get descendants of each child
          (append children
                  (apply #'append
                         (mapcar #'claude-code-ide--get-process-descendants children))))
      (error
       (claude-code-ide-debug "Error getting process descendants for PID %d: %s"
                              pid (error-message-string err))
       nil))))

(defun claude-code-ide--kill-process-tree (pid &optional remote)
  "Kill process PID and all its descendants.
If REMOTE is non-nil, it should be a remote file name for killing remote processes.
Uses SIGTERM first, then SIGKILL if process doesn't terminate."
  (let ((all-pids (append (claude-code-ide--get-process-descendants pid) (list pid))))
    (claude-code-ide-debug "Killing process tree for PID %d: %s" pid all-pids)
    (dolist (target-pid all-pids)
      (condition-case err
          (if remote
              ;; For remote processes, use TRAMP to execute kill command
              (let ((default-directory remote))
                (claude-code-ide-debug "Killing remote PID %d via TRAMP" target-pid)
                (call-process "kill" nil nil nil "-15" (number-to-string target-pid))
                (sleep-for 0.1)
                ;; Check if still alive and send SIGKILL if needed
                (when (zerop (call-process "kill" nil nil nil "-0" (number-to-string target-pid)))
                  (call-process "kill" nil nil nil "-9" (number-to-string target-pid))
                  (claude-code-ide-debug "Sent SIGKILL to remote PID %d" target-pid)))
            ;; For local processes, use signal-process
            (progn
              (signal-process target-pid 15)
              (claude-code-ide-debug "Sent SIGTERM to PID %d" target-pid)
              (sleep-for 0.1)
              (when (ignore-errors (signal-process target-pid 0) t)
                (signal-process target-pid 9)
                (claude-code-ide-debug "Sent SIGKILL to PID %d" target-pid))))
        (error
         (claude-code-ide-debug "Error killing PID %d: %s"
                                target-pid (error-message-string err)))))))

(defun claude-code-ide--kill-all-session-processes (directory)
  "Kill all processes associated with the Claude session for DIRECTORY.
This includes the main terminal process and any SSH tunnel processes."
  (claude-code-ide-debug "Killing all processes for session: %s" directory)
  (when-let* ((process (gethash directory claude-code-ide--processes)))
    (when (and (processp process) (process-live-p process))
      (let* ((buffer (process-buffer process))
             (remote (when (and buffer (buffer-live-p buffer))
                       (with-current-buffer buffer
                         (file-remote-p default-directory)))))
        ;; Remove sentinel to prevent error messages
        ...
        (when (and buffer (buffer-live-p buffer))
          (dolist (proc (process-list))
            (when (and (process-live-p proc)
                       ...
                       (string-match-p "ssh.*tunnel" (process-name proc)))
              (claude-code-ide-debug "Killing SSH tunnel: %s" (process-name proc))
              (ignore-errors (delete-process proc)))))
        ;; Kill main process tree
        (claude-code-ide--kill-process-tree (process-id process) remote)))))
