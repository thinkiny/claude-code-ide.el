...
(defcustom claude-code-ide-emacs-prompt "IMPORTANT: Connect to Emacs via claude-code-ide.el integration. Emacs uses mixed coordinates: Lines: 1-based (line 1 = first line), Columns: 0-based (column 0 = first column). Features: xref, tree-sitter, imenu, project, diagnostics, Context-aware with automatic project/file/selection tracking. Prefer Emacs tools to other tools"
